...
namespace ClientServer.Server
{
    public class Program
    {
        ...
        private static string ConnectionString { get; set; } = "amqp://admin:12345@localhost:5672";
            ...
            consumerEvent.Received += (ch, ea) =>
                {
                    ...
                    var model = JsonConvert.DeserializeObject<User>(modelJson);
        ...
        private static void WriteToQueue(string queueName, User user)
        {
            var messageArray = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(user));
